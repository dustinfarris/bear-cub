...
- **Stage kiosk states, don't hand-poke the dev database**: `BearCub.Dev.Scenarios` (`dev/bear_cub/dev/scenarios.ex`) holds one named scenario per reviewable kiosk state (`early_bird/1`, `reset/1`, `open/1` to force a routine window all day). Add a scenario when a feature adds a state worth looking at; run them through Tidewave `project_eval` so config overrides and re-render broadcasts land in the running server. A dev server started before a `config/runtime.exs` change does not see the new keys until restart — scenarios that need them set defaults in the VM.
